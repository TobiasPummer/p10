    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 20) {
                ...
                Text("Um die gezogenen Items freizuschalten, werden 50 Gems benötigt.")
                    .multilineTextAlignment(.center)
                    .foregroundColor(.white)

                HStack(spacing: 4) {
                    Text("Aktuelles Guthaben:")
                    Text("\(currencyManager.gems)")
                ...
                HStack(spacing: 15) {
                    ...
                    Button(action: {
                        if currencyManager.gems >= unlockFee {
                            currencyManager.gems -= unlockFee
                            onUnlock()
                        }
                        isPresented = false
                    }) {
                        Text("Für 50 ")
                            .font(.headline)
                            .foregroundColor(.white) +
                        Text(Image(systemSymbol: .diamondCircle))
                            .font(.headline)
                            .foregroundColor(.purple) +
                        Text(" freischalten")
                            .font(.headline)
                    ...
                    .padding()
                    .frame(maxWidth: .infinity)
                    .background(currencyManager.gems >= unlockFee ? Color.purple : Color.gray)
                    .cornerRadius(10)
                    .disabled(currencyManager.gems < unlockFee)
                }
            }
            .padding(25)
            .background(
        ...
        .overlay {
            if showUnlockAlert {
                UnlockItemsAlert(
                    ...
                    onUnlock: {
                        // Items successfully unlocked
                        dismiss()
                    },
                    onCancel: {
                        // User declined to unlock items
                        dismiss()
                    }
                )
            }
