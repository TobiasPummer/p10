...
struct RootView: View {
      ...
      @State private var showLimitedOffer: Bool = false
...
//           }
        ...
        .disabled(showMissions || showLimitedOffer)
        ...
        if showLimitedOffer {
            LimitedGachaOverview(currency: currency, isPresented: $showLimitedOffer)
                .transition(.opacity)
                .animation(.easeInOut, value: showLimitedOffer)
                .zIndex(100)
        }

        PlayerFeedBanner()
                            .animation(.easeInOut, value: showPlayerFeed)
